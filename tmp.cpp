...
// 用于对类中的字段进行遍历和处理，它会对指定对象的每个字段上进行操作
template<class TI, class U, class F> constexpr void NV_Var(U&& u, F&& f) {
...
template<class Name> struct NamedValueBase {
    using TName = Name;
...
template<class Name, class T> struct NamedValue : NamedValueBase<Name> {
    ...
    explicit constexpr NamedValue(T v)
        : value{v} {}
    template<class U> constexpr bool operator==(U v) const {
...
template<typename... Es> struct ElemList {
    ...
    explicit constexpr ElemList(Es... elems)
        : elems{elems...} {}
    ...
    template<class F> constexpr void ForEach(F&& f) const {
        Accumulate(0, [&](auto, const auto& field) {
            std::forward<F>(f)(field);
            return 0;
        });
    }
    template<class S> static constexpr bool Contains() {
        return (Es::TName::template Is<S>() || ...);
    }
    template<class F> constexpr std::size_t FindIf(F&& f) const {
        return detail::FindIf(*this, std::forward<F>(f),
                              std::make_index_sequence<size>{});
    }
    template<class S> constexpr const auto& Find() const {
    ...
    template<class T> constexpr std::size_t FindValue(const T& v) const {
    ...
    template<typename T, typename S> constexpr const T& ValueOfName(S n) const {
    ...
    template<class E> constexpr auto Push(E e) const {
        return std::apply(
            [e](auto... es) { return ElemList<Es..., E>{es..., e}; }, elems);
    }
    template<class E> constexpr auto Insert(E e) const {
...
template<class Name, class T> struct Attr : NamedValue<Name, T> {
    constexpr Attr(Name, T v)
        : NamedValue<Name, T>{v} {}
};
template<class Name> struct Attr<Name, void> : NamedValue<Name, void> {
    constexpr Attr(Name) {}
};
template<typename... As> struct AttrList : ElemList<As...> {
...
template<typename... Fs> struct FieldList : ElemList<Fs...> {
...
template<typename... Bs> struct BaseList : ElemList<Bs...> {
    constexpr BaseList(Bs... bs)
        : ElemList<Bs...>{bs...} {}
};
template<typename... Ts> struct TypeInfoList : ElemList<Ts...> {
...
template<class T, typename... Bases> struct TypeInfoBase {
    using Type = T;
    static constexpr ElemList bases{Bases{}...};
    ...
    template<class F> static constexpr void DFS_forEach(F&& f) {
    ...
    static constexpr void forEach_var(U&& obj, Func&& func) {
        vbases().ForEach([&](auto vb) {
